  ...
  */
    ...
    if (res.code !== 20000) {
      ...
      const code2Msg = new Map([
        ...
        [
          -3,
          () => {
            showToast(`Error: ${res.code}, ${t('dataInteraction.errorCodeMessage.1003', [res.message]) || res.message || 'Error'}`)
          }
        ], // 您的账号已在其他设备登录
      ])
  ...
  error => {
    ...
    if (code === 'ECONNABORTED' || message.indexOf('timeout') !== -1) {
      // 请求超时
      showToast(t('dataInteraction.errorCodeMessage.1013') || 'Error')
    } else if (message.indexOf('Network Error') !== -1) {
      // 请求错误
      showToast(t('dataInteraction.errorCodeMessage.1004') || 'Error')
    } else {
      showToast(message)
    }
    return Promise.reject(error)
